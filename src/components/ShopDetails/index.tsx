...
const ShopDetails = ({ apiProduct }: { apiProduct?: any }) => {
  ...
  // Helper to extract unique variant attribute values
  const getVariantOptions = (variants: any[] = [], attrKey: string) => {
    const values = new Set<string>();
    variants.forEach((v) => {
      if (v.attributes?.[attrKey]) values.add(v.attributes[attrKey]);
    });
  ...
  // Dynamically discover all unique attribute keys from variants
  ...
  // Build { attrKey: [{ id, title }] } for each key
  const variantAttrOptions = Object.fromEntries(
    allAttrKeys.map((key: string) => [key, getVariantOptions(product?.variants, key)])
  );
  ...
  // State with dynamic defaults from variant options
  ...
  const matchedVariant = product?.variants?.find((v: any) =>
    Object.entries(selectedAttrs).every(
      ([key, val]) => v.attributes?.[key]?.toLowerCase().replace(/\s+/g, "-") === val
    )
  );
                ...
                <div className="max-w-[539px] w-full">
                  ...
                  <div className="flex flex-wrap items-center gap-5.5 mb-4.5">
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className={displayStock > 0 ? "text-green" : "text-red"}>
                                    ...
                                    <div className="relative">
                                      ...
                                      >
                                        <span
                                          ...
                                          style={{ backgroundColor: opt.title }}
                                        />
                                      </div>
                                    </div>
                                  ) : (
